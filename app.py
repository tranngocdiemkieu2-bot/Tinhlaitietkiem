...
# Nhập dữ liệu
C = st.number_input("Nhập số tiền gửi (triệu đồng)", min_value=0.0, value=500.0, step=10.0)

i = st.number_input("Nhập lãi suất (%/năm)", min_value=0.0, value=5.0, step=0.1) / 100

ngay_gui = st.date_input("Chọn ngày gửi")

n = st.number_input("Nhập thời gian gửi (ngày)", min_value=1, value=92, step=1)
